...
bool wireless_msgs__msg__scan__convert_from_py(PyObject * _pymsg, void * _ros_message)
{
  // check that the passed message is of the expected Python class
  {
    ...
    assert(strncmp("wireless_msgs.msg._scan.Scan", full_classname_dest, 28) == 0);
  }
  wireless_msgs__msg__Scan * ros_message = _ros_message;
  {  // networks
    ...
    if (!wireless_msgs__msg__Network__Sequence__init(&(ros_message->networks), size)) {
      PyErr_SetString(PyExc_RuntimeError, "unable to create wireless_msgs__msg__Network__Sequence ros_message");
      Py_DECREF(seq_field);
      Py_DECREF(field);
      return false;
    }
    wireless_msgs__msg__Network * dest = ros_message->networks.data;
    for (Py_ssize_t i = 0; i < size; ++i) {
      if (!wireless_msgs__msg__network__convert_from_py(PySequence_Fast_GET_ITEM(seq_field, i), &dest[i])) {
        Py_DECREF(seq_field);
        Py_DECREF(field);
        return false;
      }
...
PyObject * wireless_msgs__msg__scan__convert_to_py(void * raw_ros_message)
{
  ...
  {
    PyObject * pymessage_module = PyImport_ImportModule("wireless_msgs.msg._scan");
  ...
  {  // networks
    ...
    for (size_t i = 0; i < size; ++i) {
      ...
      PyObject * pyitem = wireless_msgs__msg__network__convert_to_py(item);
      if (!pyitem) {
        Py_DECREF(field);
        return NULL;
      }
